...
from brax_dm_wrapper import DmEnvWrapper


_VALID_TASK_SUITES = ('gym', 'control', 'brax')
...
def make_environment(suite: str, task: str) -> dm_env.Environment:
  ...
  if suite == 'gym':
    ...
    env = envs.get_environment(task)
    env = DmEnvWrapper(env)
    env = wrappers.StepLimitWrapper(env, step_limit=1000)
