...
# def test_MinAtar_environment(level: str = 'Asterix'):
#     level_name = f'{level}-MinAtar'
#     env, env_params = gymnax.make(level_name)
#     wrappered_env = GymnaxToDmEnvWrapper(env)
#     return wrappered_env

def make_MinAtar_environment(
    level: str = 'Asterix',
    ...
    to_float: bool = True,
    scale_dims: Tuple[int, int] = (10, 10),
) -> dm_env.Environment:
  """Loads the MinAtar environment."""
# Internal logic.
  # version = 'v0' if sticky_actions else 'v4'
  level_name = f'{level}-MinAtar'
  env, env_params = gymnax.make(level_name)
  wrappered_env = GymnaxToGymWrapper(env)
  ...
  return wrappers.wrap_all(wrappered_env, wrapper_list)
