...
import wandb
from utils import metrics_todict
  ...
  def run(
    ...
    with signals.runtime_terminator():
      while not should_terminate(episode_count, step_count):
        ...
        # wandb.log(result, num_evaluator_episodes + episode_count)
        temp_return += result['episode_return']
        temp_length += result['episode_length']

        self._logger.write(result)
        result['episode_return'] = temp_return/episode_count
        result['episode_length'] = temp_length/episode_count
        # if episode_count==10:
        #   wandb.log(result, num_evaluator_episodes + episode_count)
        output_result = result
      wandb.log(output_result, num_evaluator_episodes + episode_count)
