...
from recorders import WandbRecorder, LogRecorder
...
def main(config, mle_log, log_ext=""):
    ...
    wandb_name = '-'.join(
        [config.train_type, config.env_name, "gymnax"]
    )
    wandb_recorder = WandbRecorder(
        project="EvoRL",
        name=wandb_name,
        config=config.toDict(),  # save the unrescaled config
        mode=None
    )
    
    # Log and store the results.
    log_steps, log_return, network_ckpt = train_fn(
        rng, config, model, params, mle_log, wandb_recorder
    )
