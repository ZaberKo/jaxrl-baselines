...
def train_ppo(rng, config, model, params, mle_log, recorder):
    ...
    for step in t:
        ...
        if step % (config.n_steps + 1) == 0:
            ...
            recorder.write(data=metric_dict,step=step,step_name="train_steps")
            batch = batch_manager.reset()
        ...
        if (step + 1) % config.evaluate_every_epochs == 0:
            ...
            recorder.write(data={"eval_return": rewards},step=total_steps,step_name="eval_steps")
